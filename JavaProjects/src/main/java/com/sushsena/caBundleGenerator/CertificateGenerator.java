package com.sushsena.caBundleGenerator;
...
public class CertificateGenerator {
	public static void generateKeyPairAndCABundle(String keyStoreFilePath, String keyStorePassword) throws Exception {
		...
		X500Principal subject= new X500Principal("CN=Sushanta, ST=Odisha, C=India");
	...
	public static void main(String[] args) {
		// ConfigurableApplicationContext context = SpringApplication.run(CertificateGenerator.class, args);
		
		// CertificateGenerator bean = context.getBean(CertificateGenerator.class);
		
		// bean.run();
		try { 
			CertificateGenerator.generateKeyPairAndCABundle("ca-keystore.jks", "changeit");
		} catch (Exception e) {
		    e.printStackTrace();
		}
